  ...
  return (
    <nav className="py-4 flex justify-between items-center gap-4">
      <div className="flex items-center gap-4">
        <div className="md:hidden text-2xl" onClick={() => setDrawerShow(true)}>
          <RiMenu2Fill />
        </div>
        <Link to='/' className="text-2xl font-medium">Pre<span className="text-primary">Wedding</span></Link>
      </div>
      <ul className="bg-white md:bg-none flex flex-col md:flex-row justify-center items-center gap-6 fixed -left-full top-0 bottom-0 md:static w-full max-w-[300px] md:w-auto md:max-w-none [box-shadow:10px_0px_30px_30px_rgba(0,0,0,0.3)] md:shadow-none me-8 md:me-0 transition-[left] [&_a]:text-xl md:[&_a]:text-base" style={drawerShow ? {left: 0} : {}}>
        <FaCircleXmark className="md:hidden text-3xl text-primary absolute right-6 top-6 cursor-progress" onClick={() => setDrawerShow(false)} />
        <li>
