  ...
  return (
    <nav className="py-4 flex justify-between items-center gap-4">
      <div className="flex items-center gap-4">
        ...
        <Link to='/' className="text-2xl font-medium" onClick={() => scrollTo(0, 0)}>Pre<span className="text-primary">Wedding</span></Link>
      </div>
        ...
        userLoaded ? !user ? <Link to='/login' className="btn btn-primary">Login</Link> : <div className="relative">
          ...
          </div>
          {
            profileShow ? <div className="bg-white text-text-color px-6 py-4 rounded-md lg:rounded-tr-none text-center w-[310px] absolute top-[calc(100%+15px)] right-0 z-10 shadow-2xl">
              <span className="block w-4 h-4 bg-white rotate-45 absolute -top-2 right-3 lg:right-[3px]"></span>
